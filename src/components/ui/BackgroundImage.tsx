...
export const BackgroundImage = memo<BackgroundImageProps>(
  ({
    ...
    return (
      <ImageBackground source={source} style={[styles.container, style]} resizeMode="cover">
        {overlay && !gradient && (
          <View
            style={[
              styles.overlay,
...
export const ScreenWithBackground = memo<ScreenWithBackgroundProps>(
  ({ imageSource, darkOverlay = true, children }) => {
...
export const HeroBackground = memo<HeroBackgroundProps>(
  ({ imageSource, height = SCREEN_HEIGHT * 0.4, children }) => {
    return (
      <ImageBackground
        source={
          imageSource || {
